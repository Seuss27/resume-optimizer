...
def load_profile(profile_csv_path):
    ...
    profile_dict = pd.Series(
        df_profile['Value'].values, 
        index=df_profile['Key']
    ).to_dict()
    
    return profile_dict

def convert_sheets_to_master_data(experience_csv_path, profile_csv_path, output_json_path='master_data.json'):
    ...
    if 'Keywords / Tech Stack' in df_experience.columns:
        ...
        flat_skills = list(set([skill.strip() for sublist in all_skills_nested for skill in sublist]))
